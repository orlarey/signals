    ...
    if (int d; isSigInt(s2, &d) && (d >= 0) && (d <= sigs::g.gMaxFIRSize)) {
        if (d > 0) {
            if (tvec V1; isSigFIR(s1, V1) && canShiftTaps(V1)) {
                ...
                return sigFIR(VR);
            } else if (Tree ck, f; isSigClocked(s1, ck, f) && isSigFIR(f) && canShiftTaps(f->branches())) {
                tvec V1 = f->branches();
                tvec VR;
                VR.push_back(V1[0]);
                for (int i = 0; i < d; i++) {
                ...
                return sigFIR(VR);
            } else if (Tree ck, f; isSigFIR(s1) || (isSigClocked(s1, ck, f) && isSigFIR(f))) {
                // a FIR with a coefficient that varies : the delay stays on its
                // result. Both spellings reach here, the bare FIR and the one
                // under a clock : the guard above refused them for the same reason.
...
{
    return sigs::simplify(sigMul(sigInt(-1), sig));
}
    ...
    if (isSigFIR(s1, V1) && isSigFIR(s2, V2)) {
        ...
        if (V1[0] == V2[0]) {
            ...
            for (unsigned int i = 1; i < minsize; i++) {
                V.push_back(sigs::simplify(sigAdd(V1[i], V2[i])));
            }
        ...
        V1[1] = sigs::simplify(sigAdd(V1[1], r));
        return sigFIR(V1);
    } else if (isSigFIR(s2, V2) && isDivisibleBy(s1, V2[0], r)) {
        // CASE 3: S*R + [S, C0, C1, ...] = [S, C0+R, C1, ...]
        V2[1] = sigs::simplify(sigAdd(V2[1], r));
        return sigFIR(V2);
    } else {
        // CASE 4: Not two FIRs
        return sigs::simplify(sigAdd(s1, s2));
    }
}
...
Tree mulSigFIR(Tree s1, Tree s2)
{
    ...
    if (isSigFIR(s1, V)) {
        ...
        if (n == 1) {
            ...
            for (unsigned int i = 1; i < V.size(); i++) {
                if (!isZero(V[i])) {
                    V[i] = sigs::simplify(sigMul(V[i], s2));
                }
            }
            return sigFIR(V);
        } else if (n > 1) {
            ...
            if (isSigInt(s2, &ii) || isSigReal(s2, &r)) {
                ...
                // CASE 2: Not a FIR
                return sigs::simplify(sigMul(s1, s2));
            }
        } else {
            // CASE 3: Not a FIR
            return sigs::simplify(sigMul(s1, s2));
    ...
    // FIN AJOUT YO
        ...
        Tree sr = sigs::simplify(sigMul(s1, s2));
...
Tree divSigFIR(Tree s1, Tree s2)
{
    ...
    if (isSigFIR(s1, V)) {
        ...
        if (n == 1) {
            ...
            for (unsigned int i = 1; i < V.size(); i++) {
                if (!isZero(V[i])) {
                    V[i] = sigs::simplify(sigDiv(V[i], s2));
                }
            }
            return sigFIR(V);
        } else if (n > 1) {
            ...
            if (isSigInt(s2, &ii) || isSigReal(s2, &r)) {
                ...
                // CASE 2: Not a FIR
                return sigs::simplify(sigDiv(s1, s2));
            }
        } else {
            return sigs::simplify(sigDiv(s1, s2));
        }
...
bool haveEquivCoefs(const tvec& V1, const tvec& V2)
{
    ...
    for (unsigned int i = 1; i < V1.size(); i++) {
        Tree c = sigs::simplify(sigSub(V1[i], V2[i]));
        if (!isZero(c)) {
            return false;
        }
    }
...
bool haveComplementaryCoefs(const tvec& V1, const tvec& V2)
{
    ...
    for (unsigned int i = 1; i < V1.size(); i++) {
        Tree c = sigs::simplify(sigAdd(V1[i], V2[i]));
        if (!isZero(c)) {
            return false;
        }
    }
...
Tree simplifyFIR(Tree sig)
{
    if (tvec V; isSigFIR(sig, V)) {
        ...
        // if (lnz == 1) {
        //     // not a real FIR
        //     return sigs::simplify(sigMul(V[0], V[1]));
        // }
        if (lnz < V.size() - 1) {
...
static std::pair<Tree, Tree> splitMulSig(Tree sig)
{
    ...
    if (Tree x, y; isSigMul(sig, x, y)) {
        ...
        return {sigs::simplify(sigMul(a, c)), sigs::simplify(sigMul(b, d))};
    }
    ...
    if (Tree x, y; isSigDiv(sig, x, y)) {
        ...
        auto [a, b] = splitMulSig(x);
        auto [c, d] = splitMulSig(y);
        return {sigs::simplify(sigDiv(a, c)), sigs::simplify(sigDiv(b, d))};
    }
...
void combine(std::map<Tree, Tree>& M, bool subflag, Tree sig)
{
    ...
    if (M.find(key) == M.end()) {
        ...
        M[key] = (subflag) ? sigs::simplify(sigSub(M[key], sig)) : sigs::simplify(sigAdd(M[key], sig));
    }
}
...
Tree combineFIRs(Tree x, Tree y, bool subflag)
{
    ...
    for (auto t : M) {
        ...
        if (tvec coefs; isSigFIR(term, coefs) && (coefs.size() == 2)) {
            ...
            term = sigs::simplify(sigMul(coefs[1], coefs[0]));
        }
        if (init) {
            result = term;
            init   = false;
