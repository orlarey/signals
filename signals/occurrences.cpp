...
static int xVariability(int v, int r)
{
    ...
    // TLIB_ASSERT(v < 3);				// kKonst=0, kBlock=1, kSamp=2
    // TLIB_ASSERT(r==0 | v==2);
    if (r > 1) {
        r = 1;
    }
...
void OccMarkup::incOcc(Tree env, int v, int r, int d, Tree xc, Tree t)
{
    ...
    if (firstVisit) {
        ...
        Tree c0 = (fConditions.find(t) == fConditions.end()) ? ::nil() : fConditions[t];
        ...
        if (isSigDelay(t, x, y)) {
            ...
            TLIB_ASSERT(v0 == kSamp);
            TLIB_ASSERT(V.size() >= 2);  // Otherwise not a proper FIR
            ...
            //     0 1  2  3  4
            TLIB_ASSERT(v0 == kSamp);
            TLIB_ASSERT(V.size() >= 4);  // IIR[_,X,C0,C1] Otherwise not a proper FIR
