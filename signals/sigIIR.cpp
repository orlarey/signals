...
Tree addSigIIR_real(Tree rt, Tree x, Tree y)
{
    ...
    if (cx && cy) {
        ...
        for (unsigned int i = 1; i < n; i++) {
            coef.push_back(sigs::simplify(sigAdd(coef1[i], coef2[i])));
        }
        ...
        coef1[1] = sigs::simplify(sigAdd(coef1[1], y));
        return sigIIR(coef1);
    } else if (cy) {
        // x is not concerned, but y is, we add x to the input signal of y
        // x + [v, b, c0, c1, c2, ...] -> [v, x+b, c0, c1, c2, ...]
        coef2[1] = sigs::simplify(sigAdd(coef2[1], x));  // add x to the input signal of the IIR
        return sigIIR(coef2);
    } else {
        // x and y are unrelated to the recursive variable
        return sigs::simplify(sigAdd(x, y));
    }
}
...
Tree subSigIIR_real(Tree rt, Tree x, Tree y)
{
    ...
    if (cx && cy) {
        ...
        // add coefficients of the same order
        for (unsigned int i = 1; i < n; i++) {
            coef.push_back(sigs::simplify(sigSub(coef1[i], coef2[i])));
        }
        // copy coefficients of the longuest IIR
        for (unsigned int i = n; i < coef1.size(); i++) {
            coef.push_back(coef1[i]);
        ...
        for (unsigned int i = n; i < coef2.size(); i++) {
            coef.push_back(sigs::simplify(sigSub(sigInt(0), coef2[i])));
        }
        return sigIIR(coef);
    } else if (cx) {
        // x is a concerned IIR but y is not, we add y to the input signal of x
        // [v, a, c0, c1, c2, ...] + y -> [v, a+y, c0, c1, c2, ...]
        coef1[1] = sigs::simplify(sigSub(coef1[1], y));
        return sigIIR(coef1);
    } else if (cy) {
        // x is not concerned, but y is, we add x to the input signal of y
        // x + [v, b, c0, c1, c2, ...] -> [v, x-b, -c0, -c1, -c2, ...]
        ...
        for (unsigned int j = 1; j < coef2.size(); j++) {
            coef2[j] = sigs::simplify(sigSub(sigInt(0), coef2[j]));
        }
        coef2[1] = sigs::simplify(sigAdd(x, coef2[1]));  // add x to the input signal of the IIR
        return sigIIR(coef2);
    } else {
        // x and y are unrelated to the recursive variable
        return sigs::simplify(sigSub(x, y));
    }
}
...
Tree mulSigIIR_real(Tree rt, Tree x, Tree y)
{
    ...
    if (cx && cy) {
        ...
        for (unsigned int i = 1; i < coef1.size(); i++) {
            coef.push_back(sigs::simplify(sigMul(coef1[i], y)));
        }
        return sigIIR(coef);
    } else if (cy) {
        ...
        for (unsigned int i = 1; i < coef2.size(); i++) {
            coef.push_back(sigs::simplify(sigMul(coef2[i], x)));
        }
        return sigIIR(coef);
    } else {
        // x and y are unrelated to the recursive variable
        return sigs::simplify(sigMul(x, y));
    }
}
...
Tree divSigIIR_real(Tree rt, Tree x, Tree y)
{
    ...
    if (cy) {
        ...
        coef.push_back(coef1[0]);  // Vi
        for (unsigned int i = 1; i < coef1.size(); i++) {
            coef.push_back(sigs::simplify(sigDiv(coef1[i], y)));
        }
        return sigIIR(coef);
    } else {
        // x and y are unrelated to the recursive variable
        return sigs::simplify(sigDiv(x, y));
    }
}
