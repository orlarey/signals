...
SIGS_API Tree sigSum(const tvec& sigs)
{
    return tree(sigs::g.SIGSUM, sigs);
}
...
SIGS_API bool isSigSum(Tree s, tvec& sigs)
{
...
{
    return tree(sigs::g.SIGTEMPVAR, s);
}

SIGS_API bool isSigTempVar(Tree s)
{
    return isTree(s, sigs::g.SIGTEMPVAR);
}

SIGS_API bool isSigTempVar(Tree s, Tree& x)
{
    return isTree(s, sigs::g.SIGTEMPVAR, x);
}

SIGS_API Tree sigPermVar(Tree s)
{
    return tree(sigs::g.SIGPERMVAR, s);
}

SIGS_API bool isSigPermVar(Tree s)
{
    return isTree(s, sigs::g.SIGPERMVAR);
}

SIGS_API bool isSigPermVar(Tree s, Tree& x)
{
    return isTree(s, sigs::g.SIGPERMVAR, x);
}

SIGS_API Tree sigZeroPad(Tree x, Tree y)
{
    return tree(sigs::g.SIGZEROPAD, x, y);
}

SIGS_API bool isSigZeroPad(Tree s)
{
    return isTree(s, sigs::g.SIGZEROPAD);
}

SIGS_API bool isSigZeroPad(Tree s, Tree& x, Tree& y)
{
    return isTree(s, sigs::g.SIGZEROPAD, x, y);
}

SIGS_API Tree sigSeq(Tree x, Tree y)
{
    return tree(sigs::g.SIGSEQ, x, y);
}

SIGS_API bool isSigSeq(Tree s)
{
    return isTree(s, sigs::g.SIGSEQ);
}

SIGS_API bool isSigSeq(Tree s, Tree& x, Tree& y)
{
    return isTree(s, sigs::g.SIGSEQ, x, y);
}

SIGS_API Tree sigOD(const tvec& sigsubs)
{
    return tree(sigs::g.SIGOD, sigsubs);
}

SIGS_API bool isSigOD(Tree s)
{
    return isTree(s, sigs::g.SIGOD);
}

SIGS_API bool isSigOD(Tree s, tvec& sigsubs)
{
    if (isTree(s, sigs::g.SIGOD)) {
        sigsubs = s->branches();
        return true;
    } else {
        return false;
...
{
    return tree(sigs::g.SIGUS, sigsubs);
}

SIGS_API bool isSigUS(Tree s)
{
    return isTree(s, sigs::g.SIGUS);
}

SIGS_API bool isSigUS(Tree s, tvec& sigsubs)
{
    if (isTree(s, sigs::g.SIGUS)) {
        sigsubs = s->branches();
        return true;
    } else {
        return false;
...
{
    return tree(sigs::g.SIGDS, sigsubs);
}

SIGS_API bool isSigDS(Tree s)
{
    return isTree(s, sigs::g.SIGDS);
}

SIGS_API bool isSigDS(Tree s, tvec& sigsubs)
{
    if (isTree(s, sigs::g.SIGDS)) {
        sigsubs = s->branches();
        return true;
    } else {
        return false;
...
{
    return isTree(s, sigs::g.SIGCLOCKED);
}

SIGS_API bool isSigClocked(Tree s, Tree& clock, Tree& y)
{
    return isTree(s, sigs::g.SIGCLOCKED, clock, y);
}

SIGS_API Tree sigClocked(Tree clock, Tree y)
{
    ...
    if (Tree h2, z; isSigClocked(y, h2, z)) {
        if (clock == h2) {
            ...
            // return y;
            return tree(sigs::g.SIGCLOCKED, clock, y);
        }
    }
...
{
    return tree(sigs::g.SIGCLOCKED, insideclkenv, tree(sigs::g.SIGCLOCKED, outsideclkenv, y));
}
...
float computeDensity(const tvec& coefs)
{
    ...
    // std::cerr << sigs::g.gSTEP << " generateFIR: "
...
Tree makeClockEnv(Tree clockenv, Tree slotenv, Tree path, Sym kind, Tree box, const siglist& lsig)
{
    Tree L = listConvert(lsig);
    // The domain kind (SIGOD/SIGUS/SIGDS) tags the clock environment; the box
    // that created the domain is kept as an opaque pointer leaf (its identity
    // distinguishes clock domains) so no traversal ever descends into it.
    Tree abox = tree(Node(kind), tree(Node((void*)box)));
...
Tree getClockenvBox(Tree clkenv)
{
    ...
    Tree addr = nth(clkenv, 3);  // The domain kind, with the box as opaque leaf
    if (void* p = nullptr; addr->arity() == 1 && isPointer(addr->branch(0)->node(), &p)) {
        return (Tree)p;
    }
    return ::nil();
}
...
Sym getClockenvKind(Tree clkenv)
{
    if (isNil(clkenv)) {
        return nullptr;
    }
    Tree addr = nth(clkenv, 3);
    Sym  kind;
    return isSym(addr->node(), &kind) ? kind : nullptr;
}

bool isODClockenv(Tree clkenv)
{
    return getClockenvKind(clkenv) == sigs::g.SIGOD;
}

bool isUSClockenv(Tree clkenv)
{
    return getClockenvKind(clkenv) == sigs::g.SIGUS;
}
bool isDSClockenv(Tree clkenv)
{
    return getClockenvKind(clkenv) == sigs::g.SIGDS;
}

Tree recTempVar(Tree clkenv1, Tree clkenv2, Tree sig)
{
    TLIB_ASSERT(isAncestorClkEnv(clkenv2, clkenv1));
    if (clkenv1 == clkenv2) {
        return sig;
    }
