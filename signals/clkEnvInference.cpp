 ...
 * @throws via tlib::error if two clkEnvs are not comparable (parallel domains)
 */
static Tree maxClkEnv(const vector<Tree>& clkEnvs)
{
    if (clkEnvs.empty()) {
    ...
    for (size_t i = 1; i < clkEnvs.size(); i++) {
        ...
        if (isAncestorClkEnv(result, c)) {
            ...
            // Incomparable: error
            tlib::error("ERROR: Incomparable clock environments\n");
        }
    }
                                      ...
                                      map<Tree, Tree>& cache)
{
    ...
    if (Tree W; isProj(sig, &p, W)) {
        ...
        TLIB_ASSERT(it != hypothesis.end());  // All recursive groups must be in hypothesis
        Tree h_w = it->second;
    ...
    if (Tree c, s; isSigClocked(sig, c, s)) {
        ...
        if (!isAncestorClkEnv(c_prime, c)) {
            tlib::error("ERROR: sigClocked(c, s) requires C⟦s⟧ ⊆ c\n");
        }
        result     = c;
        cache[sig] = result;
        return result;
    ...
    if (tvec sigsubs; isSigOD(sig, sigsubs) || isSigUS(sig, sigsubs) || isSigDS(sig, sigsubs)) {
        ...
        if (sigsubs.empty()) {
            tlib::error("ERROR: OD/US/DS has no subsignals\n");
        }
        ...
        if (!isSigClocked(clk, c_inner, h)) {
            tlib::error("ERROR: OD/US/DS requires sigClocked annotation\n");
        }
        ...
        for (size_t i = 1; i < sigsubs.size(); i++) {
            ...
            if ((yi_clk != c_inner) && !isZero(sigsubs[i])) {
                ...
                tlib::error("ERROR: OD/US/DS output not in correct domain\n");
            }
        }
    ...
    if (Tree x, y; isSigSeq(sig, x, y)) {
        ...
        if (!isAncestorClkEnv(c_x, c_y)) {
            tlib::error("ERROR: sigSeq requires c_x ⊆ c_y\n");
        }
        result     = c_x;
        cache[sig] = result;
        return result;
    ...
    if (Tree tbl, ri; isSigRDTbl(sig, tbl, ri)) {
        ...
        tlib::error("ERROR: sigRDTbl with invalid sigWRTbl\n");
    }
...
static std::map<Tree, Tree> findFixpoint(const std::set<Tree>& groups)
{
    ...
    while (!converged) {
        TLIB_ASSERT(++iteration < MAX_ITERATIONS);
...
void ClkEnvInference::annotate(Tree sigList)
{
    ...
    for (auto [sig, ce] : mapping) {
        ...
        //           << ppsig(sig) << "\n";
        sig->setProperty(sigs::g.CLKENVPROPERTY, ce);
    }
}

Tree ClkEnvInference::getClkEnv(Tree sig)
{
    Tree ce = sig->getProperty(sigs::g.CLKENVPROPERTY);
    if (!ce) {
