                            ...
                            digraph<Tree>& graph, const Tree& curSig)
{
    ...
    if (Tree clockedClk; needSubGraph(curSig, clockedClk)) {
        TLIB_ASSERT(delayed.empty());

        Tree subClkEnv, realSubClk;
        TLIB_ASSERT(isSigClocked(clockedClk, subClkEnv, realSubClk));
...
Hgraph dependenciesGraphs(Tree signalList)
{
    ...
    for (Tree sl = signalList; isList(sl); sl = tl(sl)) {
        addDependencies(::nil(), visited, sigToGraph, externals, sigToGraph[signalList],
                        hd(sl));
    }
